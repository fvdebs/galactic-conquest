...
class PlayerCalculatePointsPlugin implements TickPluginInterface
{
    /**
    ...
    {
        $this->tickRepository->playerCalculatePointsFor(
            $universe->getUniverseId()
        );

        $tickPluginResult->setAffectedRows(
            $this->tickRepository->playerUpdateRankingPositionFor(
                $universe->getUniverseId()
            )
        );
