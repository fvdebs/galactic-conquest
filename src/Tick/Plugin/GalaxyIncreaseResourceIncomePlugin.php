...
class GalaxyIncreaseResourceIncomePlugin implements TickPluginInterface
{
    /**
    ...
    {
        $this->tickRepository->galaxyUpdateResourceIncomePerTickFor(
            $universe->getUniverseId()
        );

        $tickPluginResult->setAffectedRows(
            $this->tickRepository->galaxyIncreaseResourceIncomeFor(
                $universe->getUniverseId()
            )
        );
