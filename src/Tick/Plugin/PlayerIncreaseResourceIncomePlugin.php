...
class PlayerIncreaseResourceIncomePlugin implements TickPluginInterface
{
    /**
    ...
    {
        $this->tickRepository->playerUpdateResourceIncomePerTickFor(
            $universe->getUniverseId()
        );

        $tickPluginResult->setAffectedRows(
            $this->tickRepository->playerIncreaseResourceIncomeFor(
                $universe->getUniverseId()
            )
        );
