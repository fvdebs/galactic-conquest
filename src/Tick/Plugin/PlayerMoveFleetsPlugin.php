...
class PlayerMoveFleetsPlugin implements TickPluginInterface
{
    /**
    ...
    {
        $affectedMissionComplete = $this->tickRepository->playerFleetDecreaseTicksLeftUntilMissionCompletedFor(
            $universe->getUniverseId()
        );

        $affectedMissionReach = $this->tickRepository->playerFleetDecreaseTicksLeftUntilMissionReachFor(
            $universe->getUniverseId()
        );

        $tickPluginResult->setAffectedRows(
            $affectedMissionComplete + $affectedMissionReach
        );
